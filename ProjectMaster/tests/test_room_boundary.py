"""
Test the HYBRID room + wall building-boundary detector.
...
def main():
    ...
    # The hybrid detector never reads this debug image.
    save_image(
        output_dir
        / "01_model_debug_ONLY_NOT_USED.png",
        model_debug_image,
    )
    ...
    save_image(
        output_dir
        / "02_room_door_window_debug_ONLY.png",
        object_debug,
    )
    ...
    print(
        "4. Running hybrid boundary detector V2 (physical shell first)..."
    )
    ...
    room_coordinate_image = (
        draw_room_coordinates(
            original,
            rooms,
    ...
    save_image(
        output_dir
        / "06_room_prior_mask.png",
        result.room_prior_mask,
    )

    save_image(
        output_dir
        / "07_global_closed_shell_mask.png",
        result.global_closed_shell_mask,
    )

    save_image(
        output_dir
        / "08_guided_perimeter_band_mask.png",
        result.guided_perimeter_band_mask,
    )

    save_image(
        output_dir
        / "09_guided_wall_mask.png",
        result.guided_wall_mask,
    )

    save_image(
        output_dir
        / "10_guided_closed_shell_mask.png",
        result.guided_closed_shell_mask,
    )

    save_image(
        output_dir
        / "11_selected_raw_building_mask.png",
        result.selected_raw_building_mask,
    )

    save_image(
        output_dir
        / "12_FINAL_SOLID_building_mask.png",
